...
namespace ExemploValidacao.Models
{
    public class Pessoa
    {
        ...
        [StringLength(50,MinimumLength =5, ErrorMessage = "A observacao deve ter entre 5 a 50 caracteres")]
        [Required(ErrorMessage ="Preencha a observacao")]
        public string Observacao { get; set; }

        [Range(18,50, ErrorMessage = "É preciso ter no mínimo 18 anos")]
        [Required(ErrorMessage = "Preencha a idade")]
        public int Idade { get; set; }

        [RegularExpression(@"\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$", ErrorMessage ="Email inserido nao é valido")]
